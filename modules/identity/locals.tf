...
locals {
  ...
  deploy_identity_resource_group = {
    identity = local.deploy_identity
    alerts   = local.deploy_identity
  }
  deploy_resource_groups = merge(local.deploy_network_resource_group, local.deploy_identity_resource_group)
}
...
locals {
  ...
  identity_resource_group_name = {
    identity = "${local.resource_type_names.resource_group}-${lookup(local.custom_azure_backup_geo_codes, local.location, local.location)}-${local.resource_prefix}-identity${local.resource_suffix}"
  }
  resource_group_name = merge(local.network_resource_group_name, local.identity_resource_group_name)
