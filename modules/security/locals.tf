...
locals {
  custom_settings_rsg                       = try(local.custom_settings.azurerm_resource_group["security"], local.empty_map)
  custom_settings_la_workspace              = try(local.custom_settings.azurerm_log_analytics_workspace["security"], local.empty_map)
...
locals {
  ...
  security_resource_group_name = {
    security = coalesce(
      ...
      "${local.resource_type_names.resource_group}-${lookup(local.custom_azure_backup_geo_codes, local.location, local.location)}-${local.resource_prefix}-security${local.resource_suffix}"
    )
    alerts     = "alertsRG"
  }
  resource_group_name = merge(local.network_resource_group_name, local.security_resource_group_name)
...
locals {
  log_analytics_workspace_resource_id = coalesce(
    ...
    "${local.resource_group_resource_id.security}/providers/Microsoft.OperationalInsights/workspaces/${local.azurerm_log_analytics_workspace.name}"
  )
  azurerm_log_analytics_workspace = {
    ...
    resource_group_name                = lookup(local.custom_settings_la_workspace, "resource_group_name", local.resource_group_name.security)
...
locals {
  log_analytics_solution_resource_id = {
    ...
    resource.solution_name => "${local.resource_group_resource_id.security}/providers/Microsoft.OperationsManagement/solutions/${resource.solution_name}(${local.azurerm_log_analytics_workspace.name})"
  }
  azurerm_log_analytics_solution = [
    for solution_name, solution_enabled in local.deploy_azure_monitor_solutions :
    {
      solution_name         = solution_name
      resource_group_name   = lookup(local.custom_settings_la_solution, "resource_group_name", local.resource_group_name.security)
...
# Generate the configuration output object for the security module
locals {
  module_output = {
    azurerm_resource_group = [
